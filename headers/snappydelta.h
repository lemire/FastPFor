...
public:
    void encodeArray(const uint32_t * in, const size_t length, uint32_t * out,
        ...
        snappy::RawCompress(reinterpret_cast<const char*>(in),
                         length * 4,
    ...
    const uint32_t * decodeArray(const uint32_t *in, const size_t length,
        ...
        bool ok = snappy::GetUncompressedLength(reinterpret_cast<const char*>(in), length * 4,
                                   &nvalueinbytes);
        assert(nvalueinbytes % 4 == 0);
        nvalue = nvalueinbytes / 4;
        if(!ok) throw logic_error("got some bug retrieving length");
